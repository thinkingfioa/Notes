...
#####包
```
1. 包的名称应该是层次状的,用句号分割每个部分.
```
```
2. 标准类库和一些可选的包,其名称以java和javax开头,用户创建的包名称绝不能以java和javax开头.
```
```
3. 包名称的其余部分应该包括一个或者多个描述该包的组成部分,每个组成部分应该比较短,不超过8个字符,鼓励有意义的缩写.
```
...
#####方法和域
```
方法和域的名称与类和接口的名称一样,但方法或者域的名称的第一个字母应该小写.首字母小写.
```
#####常量域
```
常量的域名称应该包含一个或者多个大写的单词,中间用下划线符合隔开.例如VALUES或NEGANTIVE_INIFINITY;常量域是一个静态的fianl域
```
```
常量域是唯一推荐使用下划线的情形
```
...
```
```
1. 对于返回boolean值的方法,其名称往往使用单词"is"开头,后面再加名词或名词短语
```
```
2. 如果方法返回别调用对象的一个非boolean的函数或者属性,通常以动词"get"开头
```
```
3. 对于转换对象类型的方法,返回不同类型的独立对象的方法,通常被称为toType.比如toString(),toArray();
```

#####总结
```
请尽量遵守标准的命名惯例,方便程序阅读和维护.
```
