...
###第52条:通过接口引用对象
```
40条中提到:应该使用接口而不是用类作为参数类型.更一般地讲,应该优先使用接口而不是类来引用对象.
```
```
如果有合适的接口类型存在,那么对于参数,返回值,变量和域来说,都应该使用接口类型进行声明
```
```
在变量声明的时候应该养成这样的习惯.Vector是List接口的一个实现
```
```java
...
#####一个注意点
```
如果原来的实现提供了某种特殊的功能,但是接口中并没有定义,则也就是说:这个功能是原来实现所特有的.那么,新的实现也要提供同样的功能.
```
#######举例
```
ThreadLocal类在Thread中使用了一个包级私有的Map域,将每个线程的值与ThreadLocal实例想关联.在1.3版本中,这个域被初始化为HashMap实例
在1.5中则改为IdentityHashMap,代替原来的HashMap实现.只需修改一行.
```
#####不存在合适的接口
```
...
#######第一种情况
```
比如:String和BigInteger类.值类很少有多个实现编写,一般都是final的.
```
#######第二种情况
```
比如:对象属于一个框架,而框架的基本类型是类,而不是接口.可以使用框架相关的基类(可能是抽象类).eg:java.util.Timer抽象类
```
#######第三种情况
```
类实现了接口,但是接口中不存在额外的方法,比如LinkedHashMap.如果程序依赖于这些额外的方法,必须使用类来引用,而不能用接口
```

#####总结
```
给定对象是否具有适当的接口应该是显然的.如果具有,则接口引用对象就会使程序更加灵活;如果不具有,则只能使用提供的最基础的类
```

###第53条:接口优先于反射机制
```
核心反射机制提供了"通过程序来访问关于已装载的类的信息"的能力.可以获得Constructor,Method和Field字段
```
```
通过调用Constructor,Method和Filed实例上的方法,可以构造底层类的实例,调用底层类的方法,并访问底层类中的域.
```
#####使用反射付出的代价
#######丧失编译时类型检查的好处
```
如果程序企图用反射方式调用不存在或则不可访问的方法,只能在运行时才能发现
```
#######执行反射访问所需要的代码非常笨拙和冗长
```
反射的代码非常乏味,阅读起来也很困难
...
###第54条:谨慎地使用本地方法
```
所谓本地方法是指用本地程序设计语言(C或C++)来编写的特殊方法.
```
######本地方法的三种用途
```
1. 提供了"访问特定于平台的机制"能力,比如访问注册表和文件锁
```
```
2. 提供访问遗留代码库的能力,从而可以访问遗留数据
```
```
3. 本地语言能提高性能,所以有利于提高系统的性能
```
...
#####使用本地方法的缺点
```
1. 本地语言不安全,所以使用本地方法的应用程序不再能免受内存毁坏的影响.
```
```
2. 本地语言和平台相关,使用本地方法的应用也不再可以自由移植.
```
```
3. 本地方法的应用程序很难调试
```
```
4. 进入和退出本地代码都需要一定的固定开销,所以如果本地代码只是做少量的工作,本地方法可能降低性能
```

#####总结
```
请在使用本地方法之前务必三思.如果程序必须使用本地方法来访问底层资源或遗留代码库,也要尽可能少用本地代码,并且进行全面的测试
```

###第55条:谨慎地进行优化
```
这个条目,建议粗略的看看,很多点没有讲清楚.可以重点看看总结
```
#####在优化方面,应该遵守的两条规则
```
1. 不要进行优化
```
```
2. (仅针对专家)---还是不要优化,也就是说,在你没有绝对清晰的优化方案前,请不要优化
```
#####设计中的几条原则
```
1. 优化的弊大于利,特别是不成熟的优化
```
```
2. 千万不要因为性能而牺牲合理的结构.要努力编写好的程序而不是快的程序.如果好的程序不够快,可以通过更换单个模块.来加速.方便快速
```
```
3. 必须在程序过程中考虑性能问题.特别是遍布全局并且限制性能的结构缺陷
```
```
4. 努力避免那些限制性能的设计决策.尤其是难以更改的组件,比如:模块之间交互关系以及模块与外界交互关系的组件.
```
#####考虑API设计决策的性能后果
```
1. 是公有的类型变成可变的,这可能会导致大量不必要的保护性拷贝(39)
```
```
2. 在适合使用复合模式的公有类中使用继承,会把这个类与它的超类永远的束缚在一起,从而限制了子类测性能(16)
```
```
3. 在API中使用实现类型而不是接口,会把它束缚在一个具体的实现上,即使出现更快的实现你也无法使用(52)
```

#####尽力提供好的API设计
```
一般而言,好的API会带来好的性能.为了获得好的性能而对API进行包装,是一种非常不好的想法.
```
```
通常想要猜出程序把时间花在哪里并不容易,不过可以使用性能剖析工具来提供帮助
```

#####总结
```
1. 不要费力去编写快速的程序,而应该努力编写好的程序,速度自然随之而来
```
```
2. 在设计系统时,特别是设计API,关联的重要部件时,一定要考虑性能因素.当构建完系统时,要测量它的性能,
如果不够快,则可以在性能剖析器的帮组下,找到性能瓶颈,在进行优化.
```
