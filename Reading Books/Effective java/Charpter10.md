...
###第73条:避免使用线程组
```
除了线程,锁和监视器之外,线程系统该提供了一个基本的抽象,即线程组.
线程组并没有提供所提及的任何安全功能,所提供的功能少之又少,一些基本功能早就废弃了.请放弃使用线程组.
```
```
在Java 1.5发行的版本之前,ThreadGroup API提供一个小功能：当线程抛出未捕捉的异常时，ThreadGroup.uncaughtException方法是获得控制权的唯一方式.
这项功能很有用,例如,为了把堆栈轨迹定向到一个特定于应用程序的日志中.
但是自从Java 1.5版本后,Thread的setUncaughtExceptionHandler方法提供同样的功能
```

#####总结
```
线程组基本已经被废弃,可以忽略线程组.提供使用线程池(68)
```
